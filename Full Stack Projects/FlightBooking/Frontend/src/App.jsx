    ...
    return (
        <Router>
            ...
            <Navbar bg="white" className="d-lg-none px-3 shadow-sm sticky-top">
                ...
                <Navbar.Brand className="fw-bold">SKYFLYER</Navbar.Brand>
            </Navbar>
...
const LogoSection = () => (
    <div className="text-center mb-5 px-3">
        <div className="p-3 rounded-circle bg-white d-inline-flex align-items-center justify-content-center mb-3 shadow" style={{ width: '60px', height: '60px' }}>
            <i className="bi bi-airplane-fill fs-3 text-primary"></i>
        </div>
        <h4 className="fw-bold mb-0">SKYFLYER</h4>
        <small style={{ color: '#64748b', fontSize: '0.75rem', letterSpacing: '2px', fontWeight: '700' }}>HUB_ADMIN_PRO</small>
    </div>
);
...
const NavigationLinks = ({ onLinkClick }) => (
    <nav className="nav flex-column gap-2 px-3">
        ...
        <NavLink to="/flights" className={({ isActive }) => `nav-link-side ${isActive ? 'active' : ''}`} onClick={onLinkClick}>
            <i className="bi bi-wind"></i> Flight Hub
        </NavLink>
        <NavLink to="/customers" className={({ isActive }) => `nav-link-side ${isActive ? 'active' : ''}`} onClick={onLinkClick}>
        ...
        <NavLink to="/bookings" className={({ isActive }) => `nav-link-side ${isActive ? 'active' : ''}`} onClick={onLinkClick}>
            <i className="bi bi-bookmark-check"></i> Bookings
        </NavLink>
    </nav>
);
...
const SidebarFooter = () => (
    <div className="position-absolute bottom-0 start-0 w-100 p-4 border-top border-secondary border-opacity-10 mt-auto">
        ...
        <p className="small text-muted mb-0" style={{ fontSize: '0.65rem' }}>&copy; 2026 SKYFLYER INC.</p>
    </div>
);
